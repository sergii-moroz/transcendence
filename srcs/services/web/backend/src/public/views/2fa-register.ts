...
export class Register2FAView extends View {
	setContent = (data: Record<string, any>) => {
			...
			<section>
				<div class="flex flex-col items-center justify-center px-6 py-8 mx-auto space-y-4">
					...
					<div class="flex items-center justify-between w-full px-6 pb-6 text-xs md:text-sm
						...
						<div class="h-px flex-grow bg-gray-300 dark:bg-gray-700 mx-2"></div>

						<div class="relative w-5 h-5 rounded-full border border-gray-300 dark:border-gray-700">
							<span class="absolute top-full left-1/2 -translate-x-1/2 mt-1 whitespace-nowrap text-gray-700">Verify</span>
						</div>

						<div class="h-px flex-grow bg-gray-300 dark:bg-gray-700 mx-2"></div>

						<div class="relative w-5 h-5 rounded-full border border-gray-300 dark:border-gray-700">
							<span class="absolute top-full left-1/2 -translate-x-1/2 mt-1 whitespace-nowrap text-gray-700">Backup</span>
						</div>
					...
					>
						...
						>
							...
							<div>
								<img src="${data.qr}" alt="2FA QR Code" class="mx-auto rounded-md" />
							</div>
							<p>Or enter the following code manually:</p>
							<p class="font-bold text-base">${formatString(data.secret)}</p>
	...
	override async prehandler() {
		const res = await this.api.getQR();
