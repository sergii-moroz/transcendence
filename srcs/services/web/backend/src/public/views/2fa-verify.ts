...
export class Verify2FAView extends View {
	setContent = () => {
				...
				<div class="flex flex-col items-center justify-center px-6 py-8 mx-auto space-y-4">
					...
					<form id="verify-2fa-form">
						...
						>
							...
							>
								<p class="mx-auto">Enter a code from Google Authenticator to make sure everything works</p>
								<div>
									<label
										for="code"
							...
							<div class="flex items-center justify-between">
								...
								<button id="btn-submit" type="submit" class="tw-btn-disabled w-24" disabled>Next</button>
							</div>
						</div>
					</form>
				</div>
	...
	setupEventListeners() {
		...
		const submitHandler = async (event: Event) => {
			...
			const res = await this.api.verify2FA(code.value)
			...
			if (res.ok) {
				return this.router.navigateTo('/settings/2fa/backup')
			} else {
				showError(data)
			}
		}
