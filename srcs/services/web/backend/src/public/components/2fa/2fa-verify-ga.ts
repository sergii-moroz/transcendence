...
export class TwoFAVerifyGA extends HTMLElement {
	...
	connectedCallback() {
		...
		this.inputElm?.addEventListener('input', this)
	}

	disconnectedCallback() {
		this.form?.removeEventListener('submit', this)
		this.inputElm?.removeEventListener('input', this)
	}
	...
	private async handleSubmit(event: Event) {
		...
		try {
			...
			const res = await API.twoFAGAVerify(code)

			if (res.success) {
				Router.navigateTo('/settings/2fa/ga/backup')
			} else {
