	...
	<form>
		...
		>
			...
			<div class="flex items-center justify-between">
				...
				<button type="submit" class="tw-btn-disabled w-24" disabled>Verify</button>
			</div>
		</div>
	</form>
`

export class TwoFALoginVerify extends HTMLElement {
	...
	private handleSubmit = async (event: Event) => {
		...
		try {
			...
			const res = await API.twoFALoginVerify(code)
